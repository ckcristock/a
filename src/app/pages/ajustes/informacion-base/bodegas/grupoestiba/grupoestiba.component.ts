...
export class GrupoestibaComponent {
  ...
  buscarEstibas(id_grupo,filtros=false){
    ...
    this.http.get(environment.ruta+'php/bodega_nuevo/get_estibas.php',{params})
      .subscribe(res=>{
  ...
  getGrupos(porFiltros=false){
    ...
    this.http.get(environment.ruta+'php/grupo_estiba/get_grupos_bodega.php',{params})
    .subscribe(res=>{
  ...
  getBodega(){
    ...
    this.http.get(environment.ruta + 'php/bodega_nuevo/get_bodega.php', {
      params: { Id_Bodega_Nuevo: this.idBodega }
    }).subscribe((data: any) => {
       console.log(data);
       this.bodega = data.Bodega;
      let mapaUrl = `${environment.ruta}IMAGENES/MAPABODEGA/${data.Bodega.Mapa}`;
      //Modal Mapa Bodega
      console.log(mapaUrl);
