...
export class NuevaFacturaCapitaComponent implements OnInit {
  ...
  public reducer3 = (accumulator, currentValue) => accumulator + parseFloat(currentValue.Total_Descuento);
  ...
  ngOnInit() {
    ...
    this.http.get(environment.ruta + 'php/factura_capita/clientes_con_contrato.php').subscribe((data: any) => {
      this.Cliente = data;
  ...
  GuardarFacturaCapita(formulario: NgForm){
    ...
    this.http.post(environment.ruta + '/php/factura_capita/guardar_factura.php', datos).subscribe((data: any) => {
      ...
      if (data.tipo === "success") {
        if (data.id) {
          window.open(environment.ruta + 'php/factura_capita/factura_capita_pdf.php?id='+data.id,'_blank');
        }
  ...
  getCuotasMod(regimen) {
    ...
    if (regimen != "" && parseInt(regimen) != 2) {
      ...
      this.http.get(environment.ruta+'php/factura_capita/cuotas_moderadora.php',{ params: { client: cliente, dep: departamento, pto: punto, mes: mes } }).subscribe((data:any)=>{
