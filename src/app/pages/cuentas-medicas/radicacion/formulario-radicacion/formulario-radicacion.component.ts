...
import { DepartamentosService } from 'src/app/pages/ajustes/parametros/departamentos/departamentos.service';
...
export class FormularioRadicacionComponent implements OnInit {
  ...
  text$.pipe(
    ...
    switchMap(term =>
      this.http.get<readonly string[]>(environment.ruta + "/php/clientes/lista_clientes.php", { params: { nom: term }}).pipe(
        tap(() => (this.searchFailedClient = false)),map(res => res['clientes']),
        catchError(() => {
  ...
  public total_facturas = (accumulator, currentValue) => accumulator + (parseFloat(currentValue.Valor_Factura));
  ...
  ListarFacturas(params = '') {
    ...
    this.http.get(environment.ruta + 'php/radicados/get_facturas_para_radicacion.php?' + params)
      .subscribe((data:any) => {
      if (data.code == 200) {
  ...
  filterByValue(array, string) {
    ...
    return array.filter(object =>
        Object.keys(object).some(k => !camposExcluidos.includes(k) && object[k].toString().toLowerCase().includes(string.toLowerCase())));
  }
  ...
  GetRegimenes():void{
    this.http.get(environment.ruta + 'php/GENERALES/regimen/get_regimenes.php')
    .subscribe((data:any) => {
  ...
  registrarRadicacion(){
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        this.http.post(environment.ruta+'php/radicados/guardar_radicado.php', data).subscribe((data:any) => {
          //console.log(data);
        });
        this._swal.show({
          icon: 'success',
