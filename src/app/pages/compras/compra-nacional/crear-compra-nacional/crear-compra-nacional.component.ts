...
export class CrearCompraNacionalComponent implements OnInit {
    ...
    public reducer = (accumulator, currentValue) => accumulator + parseFloat(currentValue.Cantidad);
    public reducer1 = (accumulator, currentValue) => accumulator + parseFloat(currentValue.Costo);
    public reducer2 = (accumulator, currentValue) => accumulator + parseFloat(currentValue.Iva_Acu);
    public reducer3 = (accumulator, currentValue) => accumulator + parseFloat(currentValue.Total);
    ...
    public user = JSON.parse(localStorage.getItem("User")).Identificacion_Funcionario;
    posicion: any = '';
    puntos: any = [];
      ...
      this.alertOption = {
        ...
        preConfirm: () => {
          return new Promise((resolve) => {
            this.GuardarCompra(this.FormCompra);
          })
        },
        allowOutsideClick: () => !swal.isLoading()
      }
  
      this.http.get(environment.ruta + 'php/comprasnacionales/proveedor_buscar.php').subscribe((data: any) => {
    ...
    ngOnInit() {
      ...
      if (params.Pre_Compra != undefined) {
        this.http.get(environment.ruta + 'php/rotativoscompras/detalle_pre_compra.php',{params: {  id:params.Pre_Compra}}).subscribe((data: any) => {
      ...
      this.http.get(environment.ruta + 'php/inventario_fisico_puntos/lista_punto_funcionario.php',{ params: { id: JSON.parse(localStorage.getItem('User')).Identificacion_Funcionario} } ).subscribe((data: any) => {
    ...
    filtros(){
      ...
      if (this.filtro_lab_com != "" || this.filtro_lab_gen != "" || this.filtro_cum) {
        ...
        this.http.get(environment.ruta + 'php/comprasnacionales/lista_productos.php?'+queryString).subscribe((data: any) => {
          this.Cargando = false;
        ...
        this.http.get(environment.ruta + 'php/comprasnacionales/lista_productos.php', { params: { nom: this.filtro_nombre } }).subscribe((data: any) => {
          this.Cargando = false;
          this.ListaProducto = data;
        });
      }
    ...
    searchProduct(pos, editar) {
      ...
      this.filtro_lab_gen = '';
      this.filtro_cum = '';
      let producto = (document.getElementById('Producto' + pos) as HTMLInputElement).value;
      ...
      if (producto != "") {
        ...
        this.http.get(environment.ruta + 'php/comprasnacionales/lista_productos.php', { params: { nom: producto } }).subscribe((data: any) => {
          this.Cargando = false;
          this.ListaProducto = data;
        });
      } else {
    ...
    GuardarCompra(formulario: NgForm) {
      ...
      if (params.Pre_Compra != undefined) {
        ...
        this.http.post(environment.ruta + '/php/rotativoscompras/actualizar_estado.php', datos).subscribe((data: any) => {
        });
      ...
      if(this.Lista_Productos.length>1){
        ...
        params.Pre_Compra != undefined ?   datos.append("id_pre_compra", params.Pre_Compra) : '';
        datos.append("modulo", 'Orden_Compra_Nacional');
        datos.append("datos", info);
        datos.append("productos", prod);
        datos.append("tipoBodega", this.TipoBodega);
  
        this.http.post(environment.ruta + '/php/comprasnacionales/guardar_compra_nacional.php', datos).subscribe((data: any) => {
        ...
        this.confirmacionSwal.title = "Error ";
        this.confirmacionSwal.text = "No se puede guardar un orden de compra si productos";
        this.confirmacionSwal.type = "error";
        this.confirmacionSwal.show();
      }
    ...
    addProduct(pos) {
      let checkbox = (document.getElementById('check' + pos) as HTMLInputElement).checked;
      let modelo: any = {
    ...
    AgregarProducto() {
      ...
      this.Productos.forEach((valor, i) => {
        ...
        if (this.Lista_Productos.length == 1 && this.Lista_Productos[0].Id_Producto == "") { // Cuando la lista de productos está vacía o inicializada por primera vez.
        ...
        } else if (this.band_editar) { // Cuando se quiere editar un producto.
        ...
        } else { // Cuando se quiere agregar nuevos productos.
  
          if (this.Lista_Productos.length > 1 && this.Lista_Productos[this.posicion].Id_Producto == "") { // Si la lista de productos estaba llena y la posicion donde se está buscando el producto estaba vacía, edita el campo actual.
            this.Lista_Productos[this.posicion].Costo = valor.Costo;
            this.Lista_Productos[this.posicion].Presentacion = valor.Presentacion;
            this.Lista_Productos[this.posicion].Id_Producto = valor.Id_Producto;
            this.Lista_Productos[this.posicion].producto = valor.producto;
            ...
            if (editar_producto) { // Si se editó un producto, se declara esta condicional como bandera para que elimine el ultimo campo en blanco y pueda añadir en la lista de productos sin problema.
              let last_position = this.Lista_Productos.length - 1;
              this.Lista_Productos.splice(last_position, 1);
              editar_producto = false;
            }
  
            this.Lista_Productos.push({ // Se agregan nuevos productos.
      ...
      if (this.Lista_Productos[pos].producto != "") { // Si en la ultima posición del Array ya no es vacío se agrega un nuevo campo para una nueva busqueda.
        this.Lista_Productos.push({
