...
export class AlistamientoCrearComponent implements OnInit {
  ...
  @ViewChild('confirmacionGuardarMaterial')
  private confirmacionGuardarMaterial: SwalComponent;
  @ViewChild('confirmacionGuardarFase1')
  private confirmacionGuardarFase1: SwalComponent;
  @ViewChild('confirmacionGuardarFase2')
  private confirmacionGuardarFase2: SwalComponent;
  ...
  constructor(
    ...
    this.http
      .get(environment.ruta + 'php/balanza/estado.php')
      .subscribe((data: any) => {
  ...
  buscarRem(id, tipo) {
    this.http
      .get(environment.ruta + 'php/remision_nuevo/remision.php', {
        params: { id: id },
      })
      .subscribe((data: any) => {
        console.log(data);
        ...
        if (this.origen) {
          this.mapaUrl = `${environment.ruta}IMAGENES/MAPABODEGA/${data.Origen.Mapa}`;
        } else {
          this.mapaUrl = '';
        }
          ...
          if (this.Paso === 1) {
            if (localStorage.getItem('combo')) {
              ...
              this.Consulta1 = timer(600).subscribe(() => {
                ...
                this.http
                  .get(environment.ruta + 'php/alistamiento/peso.php', {
  ...
  buscarDevolucion(id, tipo) {
    this.http
      .get(environment.ruta + 'php/noconforme/devolucion_alistamiento.php', {
        params: { id: id },
      })
      .subscribe((data: any) => {
        // console.log(data);
 ...
 */
  ...
  validar(remision, tipo) {
    ...
    if (
      parseInt(JSON.parse(localStorage.getItem('Id_Remision'))) ==
        parseInt(id) &&
      ...
      this.Peso_Total_Remision = JSON.parse(
        localStorage.getItem('Peso_Total_Remision')
      );
    } else {
      ...
      this.http
        .get(
          environment.ruta + 'php/alistamiento_nuevo/productos_remision.php',
          {
            params: { id: id, tipo },
          }
        )
        .subscribe((data: any) => {
          ...
          localStorage.setItem(
            'Tolerancia',
            JSON.stringify(this.Tolerancia_Global)
          );
        });
    }
  }
  ...
  ActualizarLista() {
    let id = this.route.snapshot.params['id'];
    this.http
      .get(environment.ruta + 'php/alistamiento_nuevo/productos_remision.php', {
        params: { id: id },
      })
      .subscribe((data: any) => {
        ...
        for (let index = 0; index < this.Lista_Productos.length; index++) {
          if (this.Lista[index] == undefined) {
            ...
            if (
              this.Lista_Productos[index]['Id_Producto'] ==
                this.Lista[index]['Id_Producto'] &&
              this.Lista_Productos[index]['Lote'] ==
                this.Lista[index]['Lote'] &&
              this.Lista_Productos['Fecha_Vencimiento'] ==
                this.Lista['Fecha_Vencimiento']
            ) {
  ...
  GuardaBalanza() {
    ...
    this.balanza = (
      document.getElementById('balanza') as HTMLInputElement
    ).value;
    ...
    if (this.Lista_Productos[this.pos]['Estiba_Validado']) {
      if (this.Lista_Productos[this.pos]['Codigo_Validado'] === false) {
        if (codigo != '') {
          ...
          this.http
            .get(
              environment.ruta + 'php/alistamiento_nuevo/consulta_codigo.php',
              {
                params: {
                  codigo: codigo,
                  mod,
            ...
            .subscribe((data: any) => {
              if (data) {
                (
                  document.getElementById('fila' + this.pos) as HTMLInputElement
                ...
                this.Lista_Productos[this.pos]['Clase'] = this.color
                  ? 'habilitado'
                  : 'habilitado2';
                (
                  document.getElementById('CodigoBarra') as HTMLInputElement
                ).value = '';
                localStorage.setItem(
                  'Lista_Producto_Inicial',
                  JSON.stringify(this.Lista_Productos)
      ...
      this.http
        .get(
          environment.ruta +
            'php/alistamiento_nuevo/consulta_codigo_estiba.php',
          {
            params: {
              codigo: codigo,
              id: this.Lista_Productos[this.pos].Id_Estiba,
            },
          }
        )
        .subscribe((data: any) => {
          ...
          if (data) {
            ...
            this.confirmacionSwal.text =
              'El codigo de barras de la estiba es incorrecto';
            this.confirmacionSwal.type = 'error';
            this.confirmacionSwal.show();
          }
        });
  ...
  open(index: number): void {
    let album = {
      src:
        environment.ruta +
        'IMAGENES/PRODUCTOS/' +
        this.Lista_Productos[index].Imagen,
      caption: 'test',
      thumb:
        environment.ruta +
        'IMAGENES/PRODUCTOS/' +
  ...
  QuitarObservable() {
    ...
    this.Tolerancia_Global = parseInt(
      JSON.parse(localStorage.getItem('Tolerancia'))
    );
    this.Consulta2 = timer(1000).subscribe(() => {
      console.log('1111');

      this.http
        .get(environment.ruta + '/php/alistamiento/peso_total.php', {
          params: { balanza: this.balanza },
        })
        .subscribe((data: any) => {
          this.Peso = data.Peso;
  ...
  HablilitarPesoSiguiente(peso) {
    ...
    if (longitud > this.pos1) {
      if (this.Lista_Productos[this.pos1]['Validado'] === false) {
        this.http
          .get(environment.ruta + 'php/alistamiento/consultar_peso.php', {
            params: {
          ...
          .subscribe((data: any) => {
            ...
            this.tolerancia = parseInt(
              this.Lista_Productos[this.pos1]['Tolerancia_Individual']
            );
            ...
            if (
              ...
              localStorage.setItem(
                'Peso_Total_Remision',
                JSON.stringify(this.Peso_Total_Remision)
              );
              (
                document.getElementById('peso' + this.pos1) as HTMLInputElement
              ).setAttribute('class', 'label-success');
              this.pos1 += 1;
              this.confirmacionSwal.title = 'Peso Correcto';
              this.confirmacionSwal.text =
  ...
  GuardarFaseI() {
    ...
    this.http
      .post(
        environment.ruta + 'php/alistamiento_nuevo/guardar_fase1.php',
        datos
      )
      .subscribe((data: any) => {
  ...
  GuardarMaterial() {
    ...
    datos.append('funcionario', this.User.Identificacion_Funcionario);
    this.http
      .post(environment.ruta + 'php/alistamiento/guardar_material.php', datos)
      .subscribe((data: any) => {
        this.confirmacionSalir.title = 'Operación Exitosa';
        this.confirmacionSalir.text = data.mensaje;
        this.confirmacionSalir.icon = data.tipo;
  ...
  GuardarFaseII(formulario: NgForm) {
    ...
    this.http
      .post(
        environment.ruta + 'php/alistamiento_nuevo/guardar_fase2.php',
        datos
      )
      .subscribe((data: any) => {
        ...
        if (data.tipo == 'success') {
          window.open(
            environment.ruta +
              '/php/archivos/descarga_zebra.php?id=' +
              this.id_remi
          );
          localStorage.removeItem('Lista_Producto_Inicial');
          localStorage.removeItem('Tolerancia');
