...
export class InventarioEstibaComponent implements OnInit {
  public InventarioFisicoModel: InventarioFisicoModel =
  ...
  async ngOnInit() {
    let params: any = {};
    params.Id_Doc_Inventario_Fisico =
      this.route.snapshot.params['idInventarioEstiba'];

    this.http
      .get(
        environment.ruta + 'php/inventariofisico/estiba/get_inventario.php',
        { params }
      )
      .subscribe(async (res: any) => {
        ...
        if (res.Tipo == 'success') {
          ...
          if (res.Data.Estado != 'Pendiente Primer Conteo') {
            ...
            this.http.post(
              environment.ruta +
                'php/inventariofisico/estiba/gestion_de_estado.php',
              datos
            );
  ...
  ConsultaCodigo(codigo) {
    ...
    let resultado = this.Productos.find(
      (lote) => lote.Codigo_Barras === codigo
    );
    ...
    if (!resultado) {
      this.http
        .get(
          environment.ruta +
            'php/inventariofisico/estiba/consulta_producto.php',
          { params }
        )
        .subscribe((data: any) => {
  ...
  ValidarLote(pos) {
    if (this.Producto_Barras['Lotes'][pos].Lote.length < 4) {
      ...
      this.Producto_Barras['Lotes'][pos].Lote =
        this.Producto_Barras['Lotes'][pos].Lote.trim();
      this.Producto_Barras['Lotes'][pos].Lote =
  ...
  VerificarSiExisteEnFila(pos) {
    ...
    if (this.Producto_Barras['Lotes'].length > 1) {
      ...
      for (
        let index = 0;
        index < this.Producto_Barras['Lotes'].length - 1;
        index++
      ) {
        if (
          this.Producto_Barras['Lotes'][pos].Lote ==
            this.Producto_Barras['Lotes'][index].Lote &&
          pos != index
        ) {
          console.log(
            'encontre coincidencias',
            this.Producto_Barras['Lotes'][index].Lote
          );
          let swal = {
            icon: 'error',
  ...
  ValidarFecha(pos) {
    ...
    var fecha_limite = new Date(
      this.Fecha_Hoy.setDate(this.Fecha_Hoy.getDate() + 3650)
    );
  ...
  ValidarCantidad(pos) {
    ...
    let cantidad =
      this.Producto_Barras['Lotes'][pos].Cantidad_Encontrada.toString();
    if (
      cantidad == '' ||
      this.Producto_Barras['Lotes'][pos].Fecha_Vencimiento == '' ||
      this.Producto_Barras['Lotes'][pos].Lote == ''
  ...
  guardarvalidado() {
    ...
    for (
      let producto = 0;
      producto < this.Producto_Barras['Lotes'].length - 1;
      producto++
    ) {
      if (
      ...
      if (this.Producto_Barras['Lotes'][producto]['Codigo'] != '') {
        if (
          this.Producto_Barras['Lotes'][producto]['Cantidad_Encontrada'] <= 0
        ) {
          {
            let swal = {
              icon: 'error',
  ...
  GuardaLotes() {
    ...
    datos.append(
      'Id_Doc_Inventario_Fisico',
      this.Model['Id_Doc_Inventario_Fisico']
    );
  ...
  SaveLoteEstiba(data: FormData) {
    return this.http.post(
      environment.ruta + 'php/inventariofisico/estiba/agrega_productos.php',
      data
    );
  }
  ...
  mostrarUltimo(producto) {
    this.Productos[producto].Lotes[
      this.Productos[producto].Lotes.length - 1
    ].AgregarLote = 'block';
  }
  ...
  reValidarLote(producto, lote) {
    ...
    if (this.Productos[producto].Lotes[lote].Lote.length < 4) {
      ...
      this.Productos[producto].Lotes[lote].Lote =
        this.Productos[producto].Lotes[lote].Lote.trim();
      this.Productos[producto].Lotes[lote].Lote =
  ...
  reValidarCantidad(producto, lote) {
    ...
    //antes de pasar a agregar a fila y guardar debemos verificar si todos los campos anteriores no están vacios
    let cantidad =
      this.Productos[producto].Lotes[lote].Cantidad_Encontrada.toString();
    if (
      cantidad == '' ||
  ...
  reVerificarSiExisteEnFila(producto, lote) {
    ...
    if (this.Productos[producto].Lotes.length > 1) {
      ...
      for (
        let index = 0;
        index < this.Productos[producto].Lotes.length - 1;
        index++
      ) {
        //SI el producto con el lote correspondiente es
        if (
          this.Productos[producto].Lotes[lote].Lote ==
            this.Productos[producto].Lotes[index].Lote &&
          this.Productos[producto].Lotes[lote] !=
            this.Productos[producto].Lotes[index]
        ) {
          this.Productos[producto].Lotes[lote].Lote = '';
          this.Productos[producto].Lotes[lote].Fecha_Vencimiento = '';
  ...
  AjustarInventario() {
    for (let producto = 0; producto < this.Productos.length; producto++) {
      for (
        let lote = 0;
        lote < this.Productos[producto]['Lotes'].length;
        lote++
      ) {
  ...
  AjustarInventarioEstiba(data: FormData) {
    return this.http.post(
      environment.ruta + 'php/inventariofisico/estiba/ajustar_inventario.php',
      data
    );
  }
  ...
  GestionarEstado(data: FormData) {
    return this.http.post(
      environment.ruta + 'php/inventariofisico/estiba/gestion_de_estado.php',
      data
    );
  }
