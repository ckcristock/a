...
export class CrearFacturaAdministrativaComponent implements OnInit {
  ...
  public reducer2 = (accumulator, currentValue) =>
    accumulator + parseFloat(currentValue.Iva);
  ...
  GuardarFactura() {
    if (this.validarCamposGeneral()) {
      ...
      if (this.Switch_Activos == 'Si') {
        ...
        productos = JSON.stringify(
         this.Lista_Factura.slice(0, this.Lista_Factura.length - 1)
       );
      ...
      this.http
        .post(
          environment.ruta +
            "/php/factura_administrativa/guardar_factura_administrativa.php",
          datos
        )
        .subscribe((data: any) => {
  ...
  getCuotasMod(regimen) {
    ...
    if (regimen != "" && parseInt(regimen) != 2) {
      let cliente = (document.getElementById("Id_Cliente") as HTMLInputElement)
        .value;
      let departamento = (document.getElementById(
        "Id_Departamento"
  ...
  validarCamposGeneral(): boolean {
    ...
    if (this.Lista_Factura && !error) {
      for (
        let producto = 0;
        producto < this.Lista_Factura.length - 1;
        producto++
      ) {
